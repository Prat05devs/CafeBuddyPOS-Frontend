...
export const OrderManagement = ({
  ...
  const getOrdersByStatus = (status?: Order['status'] | 'pending') => {
    if (!status) return orders;
    if (status === 'pending') {
      // Show all active orders (pending, preparing, ready)
      return orders.filter(order => order.status === 'pending' || order.status === 'preparing' || order.status === 'ready');
    }
  ...
  const statusTabs = [
    ...
    { value: 'pending', label: `Active (${getStatusCount('pending') + getStatusCount('preparing') + getStatusCount('ready')})`, color: 'warning' },
    { value: 'served', label: `Completed (${getStatusCount('served')})`, color: 'success' },
  ];
        ...
        <div className="mt-6">
          {statusTabs.map(tab => (
            <TabsContent key={tab.value} value={tab.value} className="mt-0">
              <OrderList
                orders={tab.value === 'all' ? orders : getOrdersByStatus(tab.value as Order['status'] | 'pending')}
...
const OrderCard: React.FC<OrderCardProps> = ({
  ...
  const getNextStatus = (currentStatus: Order['status']): Order['status'] | null => {
    ...
    if (!currentStatus) return 'served';
    
    // Simplified workflow - any active order can be marked as served/paid
    if (currentStatus === 'pending' || currentStatus === 'preparing' || currentStatus === 'ready') {
      return 'served';
    }
    
    // If already served or cancelled, no next status
    return null;
  };
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          {nextStatus && (
            <Button
              size="sm"
              onClick={() => {
                console.log('Mark Served button clicked for order:', order.id, 'Next status:', nextStatus);
                onUpdateStatus(order.id, nextStatus);
              }}
              className="flex-1 text-xs sm:text-sm bg-success hover:bg-success/90"
            >
